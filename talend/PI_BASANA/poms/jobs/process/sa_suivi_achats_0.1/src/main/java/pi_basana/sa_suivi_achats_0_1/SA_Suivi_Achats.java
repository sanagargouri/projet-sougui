...
package pi_basana.sa_suivi_achats_0_1;
...
/**
 * Job: SA_Suivi_Achats Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class SA_Suivi_Achats implements TalendJob {
	...
	private final String jobName = "SA_Suivi_Achats";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					SA_Suivi_Achats.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(SA_Suivi_Achats.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_PI_BASANA_SA_Suivi_Achats = new byte[0];
		static byte[] commonByteArray_PI_BASANA_SA_Suivi_Achats = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PI_BASANA_SA_Suivi_Achats.length) {
					if (length < 1024 && commonByteArray_PI_BASANA_SA_Suivi_Achats.length == 0) {
						commonByteArray_PI_BASANA_SA_Suivi_Achats = new byte[1024];
					} else {
						commonByteArray_PI_BASANA_SA_Suivi_Achats = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PI_BASANA_SA_Suivi_Achats, 0, length);
				strReturn = new String(commonByteArray_PI_BASANA_SA_Suivi_Achats, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PI_BASANA_SA_Suivi_Achats, 0, length);
				strReturn = new String(commonByteArray_PI_BASANA_SA_Suivi_Achats, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PI_BASANA_SA_Suivi_Achats) {
		...
		public String toString() {
			...
			sb.append(",FODEC=" + String.valueOf(FODEC));
			sb.append(",Montant_TTC=" + Montant_TTC);
			sb.append(",Retenue=" + String.valueOf(Retenue));
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:A0wlGsg+dEnTwL0BeNX4JIx92FHPvsg2kAlJ7gow10zfwpq+");
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:nXojDROcTFAi6CaGHqsphFOkIcTbVgklrsFNYw==");
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Timbre = ParserUtils.parseTo_Float(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.FODEC = ParserUtils.parseTo_Float(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Retenue = ParserUtils.parseTo_Float(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Reste_du = ParserUtils.parseTo_Float(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
...
// Start of branch "row1"
	...
	public static void main(String[] args) {
		final SA_Suivi_Achats SA_Suivi_AchatsClass = new SA_Suivi_Achats();

		int exitCode = SA_Suivi_AchatsClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = SA_Suivi_Achats.class.getClassLoader()
					.getResourceAsStream("pi_basana/sa_suivi_achats_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = SA_Suivi_Achats.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : SA_Suivi_Achats");
		}
...
/************************************************************************************************
 * 75731 characters generated by Talend Open Studio for Data Integration on the
 * 9 février 2026 à 09:25:25 CET
 ************************************************************************************************/