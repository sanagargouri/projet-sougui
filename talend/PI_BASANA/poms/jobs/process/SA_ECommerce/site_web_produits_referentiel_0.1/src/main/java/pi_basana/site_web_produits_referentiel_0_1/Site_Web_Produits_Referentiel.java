...
package pi_basana.site_web_produits_referentiel_0_1;
...
/**
 * Job: Site_Web_Produits_Referentiel Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Site_Web_Produits_Referentiel implements TalendJob {
	...
	private final String jobName = "Site_Web_Produits_Referentiel";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Site_Web_Produits_Referentiel.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Site_Web_Produits_Referentiel.this,
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_PI_BASANA_Site_Web_Produits_Referentiel = new byte[0];
		static byte[] commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel.length) {
					if (length < 1024 && commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel.length == 0) {
						commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel = new byte[1024];
					} else {
						commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel, 0, length);
				strReturn = new String(commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel, 0, length);
				strReturn = new String(commonByteArray_PI_BASANA_Site_Web_Produits_Referentiel, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PI_BASANA_Site_Web_Produits_Referentiel) {

				try {

					int length = 0;

					this.Referentiel = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.Referentiel, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("Referentiel=" + Referentiel);
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:E17vfT7BKNs2T2VCJzz0H+bn6F8UlJI+bol9sSSI5wTz/pT3");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "SiteWeb_Produits_Referentiel";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "SiteWeb_Produits_Referentiel";
				}
				int count_tDBOutput_1 = 0;

				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1
							+ "]([Referentiel] VARCHAR(255)  ,[Valeur] VARCHAR(255)  ,[Nombre] INT )");
				}
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([Referentiel],[Valeur],[Nombre]) VALUES (?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:KUuDDH+zuYNPSjmlcznLdti7D/bHfocBzr9MvA==");
				...
				try {
					...
					sheetList_tFileInputExcel_1.addAll(
							regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, "Referentiels", false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									curColName_tFileInputExcel_1 = "Referentiel";

									row1.Referentiel = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.Referentiel = null;
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Nombre = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.Referentiel == null) {
									pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(1, row1.Referentiel);
								}

								if (row1.Valeur == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, row1.Valeur);
								}

								if (row1.Nombre == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(3, row1.Nombre);
								}
	...
	public static void main(String[] args) {
		final Site_Web_Produits_Referentiel Site_Web_Produits_ReferentielClass = new Site_Web_Produits_Referentiel();

		int exitCode = Site_Web_Produits_ReferentielClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Site_Web_Produits_Referentiel.class.getClassLoader().getResourceAsStream(
					"pi_basana/site_web_produits_referentiel_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Site_Web_Produits_Referentiel.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : Site_Web_Produits_Referentiel");
		}
...
/************************************************************************************************
 * 59227 characters generated by Talend Open Studio for Data Integration on the
 * 10 février 2026 à 11:06:16 CET
 ************************************************************************************************/